...
namespace AIEmbodiment
{
    ...
    public class PersonaSession : MonoBehaviour
    {
        ...
        /// </summary>
        [SerializeField] private bool _useNativeFunctionCalling = true;

        /// <summary>Public accessor for the native function calling flag.</summary>
        public bool UseNativeFunctionCalling
        {
            get => _useNativeFunctionCalling;
            set => _useNativeFunctionCalling = value;
        }
        ...
        /// <summary>Fires when the AI finishes producing audio output.</summary>
        public event Action OnAISpeakingStopped;

        /// <summary>
        /// Fires for each raw audio chunk received from the AI.
        /// Subscribe to this to implement custom audio buffering or playback.
        /// If subscribed, you may want to set the AudioPlayback field to null in the Inspector
        /// to prevent PersonaSession from also playing audio directly.
        /// </summary>
        ...
        private void HandleAudioCaptured(float[] chunk)
        {
            ...
            // Debug log every 100 chunks to verify mic input and signal level
            if (Time.frameCount % 100 == 0)
            {
                float avgAmp = 0f;
                foreach (var s in chunk) avgAmp += Mathf.Abs(s);
                avgAmp /= chunk.Length;
                Debug.Log($"PersonaSession: Sending audio chunk to Gemini. Size: {chunk.Length}, Avg Amp: {avgAmp:F4}");
            }
        }

        private void HandleGeminiEvent(GeminiEvent ev)
        {
            // Debug.Log($"PersonaSession: Received Event {ev.Type}"); // Verbose but helpful
            switch (ev.Type)
            {
                ...
                case GeminiEventType.Audio:
                    // Debug.Log("PersonaSession: Received Audio from Gemini");
                    HandleAudioEvent(ev);
                    break;
                ...
                case GeminiEventType.OutputTranscription:
                    Debug.Log($"PersonaSession: Output Transcription: {ev.Text}");
                    HandleOutputTranscription(ev.Text);
                    break;

                case GeminiEventType.InputTranscription:
                    Debug.Log($"PersonaSession: Input Transcription: {ev.Text}");
                ...
                case GeminiEventType.Disconnected:
                    ...
                    {
                        Debug.LogError($"PersonaSession: Disconnected by server. Reason: {ev.Text}");
        ...
        private void HandleAudioEvent(GeminiEvent ev)
        {
            ...
            if (_ttsProvider == null)
            {
                // Fire raw audio event -- subscribers can buffer/play as needed
                OnAudioReceived?.Invoke(ev.AudioData);

                // Only play directly if nobody else is handling audio via OnAudioReceived
                ...
                // Always route to PacketAssembler for sync packet correlation
        ...
        private void HandleOutputTranscription(string text)
        {
            ...
            // Strip [CALL: ...] tags before broadcasting -- subscribers should only
            // see clean natural speech, not prompt-function-call syntax.
            string cleanText = !UseNativeFunctionCalling && _functionRegistry.HasRegistrations
                ? FunctionCallPattern.Replace(text, "")
                : text;

            if (!string.IsNullOrEmpty(cleanText))
            {
                OnOutputTranscription?.Invoke(cleanText);
                OnTextReceived?.Invoke(cleanText);
            }
            ...
            // Route to PacketAssembler for sync packet correlation (use cleanText so
            // SyncPacket.Text is also free of [CALL: ...] tags for TTS sentence-by-sentence mode)
            _packetAssembler?.AddTranscription(cleanText);

            // TTS text accumulation (for any active TTS provider) -- use cleanText so
            // [CALL: ...] tags are never synthesized as speech.
            if (_ttsProvider != null)
            {
                _ttsTextBuffer.Append(cleanText);
            }
        ...
        private async void SynthesizeAndEnqueue(string text)
        {
            ...
            try
            {
                ...
                if (result.HasAudio)
                {
                    ...
                    // Fire raw audio event -- subscribers (like ConversationController) can buffer/play as needed
                    OnAudioReceived?.Invoke(result.Samples);

                    // Only play directly if nobody else is handling audio via OnAudioReceived
                    if (OnAudioReceived == null && _audioPlayback != null)
                    {
        ...
        private void DispatchFunctionCall(SyncPacket packet)
        {
            ...
            // Always send a tool response when there is a callId, even if the handler
            // returned null. Gemini requires a response to complete the function call turn.
            // Use empty dict as a no-op acknowledgement when handler has no return value.
            if (packet.FunctionId != null)
            {
                SendFunctionResponse(packet.FunctionName, result ?? new Dictionary<string, object>(), packet.FunctionId);
            }
        }
