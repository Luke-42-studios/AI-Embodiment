...
namespace AIEmbodiment
{
    ...
    public class GeminiLiveClient : IDisposable
    {
        ...
        public void SendAudioStreamEnd()
        {
            ...
            UnityEngine.Debug.Log("GeminiLiveClient: Sending audioStreamEnd signal to server.");
            var payload = new JObject
            {
                ["realtimeInput"] = new JObject
                {
        ...
        private async Task SendSetupMessage()
        {
            ...
            UnityEngine.Debug.Log($"GeminiLiveClient: Sending setup message: {setup.ToString(Newtonsoft.Json.Formatting.None)}");
        ...
        private async Task ReceiveLoop(CancellationToken ct)
        {
                ...
                while (!ct.IsCancellationRequested && _ws?.State == WebSocketState.Open)
                {
                    ...
                    do
                    {
                        ...
                        if (result.MessageType == WebSocketMessageType.Close)
                        {
                            _connected = false;
                            _setupComplete = false;
                            UnityEngine.Debug.LogError($"GeminiLiveClient: WebSocket closed by server. Status: {result.CloseStatus}, Description: {result.CloseStatusDescription}");
            ...
            catch (Exception ex)
            {
                _connected = false;
                _setupComplete = false;
                UnityEngine.Debug.LogError($"GeminiLiveClient: ReceiveLoop error: {ex}");
        ...
        {
            // Log ALL incoming JSON from server (verbose -- disable once issue is found)
            UnityEngine.Debug.Log($"GeminiLiveClient: Received: {json}");
            ...
            {
                UnityEngine.Debug.LogError($"GeminiLiveClient: JSON parse error: {ex.Message}\nJSON: {json}");
                Enqueue(new GeminiEvent { Type = GeminiEventType.Error, Text = "JSON parse error: " + ex.Message });
                return;
            }

            // Check for top-level error (API error)
            if (msg["error"] != null)
            {
                var errorMsg = msg["error"]?.ToString();
                UnityEngine.Debug.LogError($"GeminiLiveClient: Server returned error: {errorMsg}");
                Enqueue(new GeminiEvent { Type = GeminiEventType.Error, Text = $"Server error: {errorMsg}" });
                return;
            }
            ...
            {
                UnityEngine.Debug.Log("GeminiLiveClient: Setup complete. Session connected.");
                _setupComplete = true;
            ...
            if (toolCall != null)
            {
                // UnityEngine.Debug.Log($"GeminiLiveClient: Received tool call: {toolCall}");
            ...
            if (msg["goAway"] != null)
            {
                UnityEngine.Debug.LogWarning("GeminiLiveClient: Received goAway from server");
                Enqueue(new GeminiEvent
                {
