...
namespace AIEmbodiment
{
    ...
    public class AudioCapture : MonoBehaviour
    {
        ...
        private IEnumerator RequestPermissionAndCapture()
        {
            ...
            {
                Debug.LogError("AudioCapture: Microphone permission denied.");
                OnPermissionDenied?.Invoke();
                yield break;
            }
#endif
            ...
            Debug.Log($"AudioCapture: Available microphones: [{string.Join(", ", Microphone.devices)}]");
            Debug.Log($"AudioCapture: Starting microphone: {device ?? "Default"}");
            ...
            Debug.Log("AudioCapture: Microphone started recording.");
            _isCapturing = true;
            _lastSamplePos = 0;
        ...
        private IEnumerator CaptureLoop()
        {
            ...
            while (_isCapturing)
            {
                ...
                while (samplesToRead >= CHUNK_SAMPLES)
                {
                    ...
                    // Diagnostic: check if mic is actually capturing audio
                    float peak = 0f;
                    for (int i = 0; i < buffer.Length; i++)
                        if (Mathf.Abs(buffer[i]) > peak) peak = Mathf.Abs(buffer[i]);
                    if (peak < 0.001f)
                        Debug.LogWarning($"AudioCapture: Chunk is near-silent (peak={peak:F5}). Check mic permissions and device selection.");
