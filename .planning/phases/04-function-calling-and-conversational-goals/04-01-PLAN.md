...
1. `void Register(string name, FunctionDeclaration declaration, FunctionHandler handler)`
   - Throws `InvalidOperationException` if `_frozen` (anti-pattern from Research: registering after Connect)
   - Validates that `name` is not null/empty (throws `ArgumentException`)
   - Stores the declaration+handler pair keyed by `name`
   - **IMPORTANT -- name duplication risk:** `FunctionDeclaration.Name` is private in the Firebase SDK, so we cannot validate that the `name` parameter matches the name passed to the `FunctionDeclaration` constructor. Add an XML doc `<remarks>` warning:
     ```
     /// <remarks>
     ...
     /// </remarks>
     ```
   - Also mention this in the `<summary>`: "The name must match the FunctionDeclaration's constructor name."
...
Follow existing code style: full XML doc comments on all public members, `AIEmbodiment` namespace.
  ...
  <verify>File exists at Packages/com.google.ai-embodiment/Runtime/FunctionRegistry.cs. Contains FunctionHandler delegate, FunctionRegistry class with Register, BuildTools, TryGetHandler, MarkCancelled, IsCancelled methods and HasRegistrations property. Register throws InvalidOperationException when frozen. BuildTools produces Tool[] from declarations. Register has XML doc remarks warning about name duplication risk with FunctionDeclaration.</verify>
  <done>FunctionRegistry maps function names to FunctionDeclaration+handler pairs, freezes after BuildTools(), tracks pending call cancellations, produces Tool[] for the Firebase GetLiveModel call, and documents the name mismatch risk in XML remarks.</done>
</task>
...
- FunctionRegistry.Register has XML doc remarks documenting the name duplication foot-gun
