...
must_haves:
  truths:
    ...
    - "3 beat assets exist with distinct time budgets, goals, director notes, ordered scene lists, and escalating urgency levels (LOW, MEDIUM, HIGH)"
    ...
    - "GoalUrgency enum (Low, Medium, High) exists and each beat asset carries an urgency value reflecting narrative escalation"
  ...
  artifacts:
    - path: "Assets/AyaLiveStream/NarrativeBeatConfig.cs"
      provides: "NarrativeBeatConfig ScriptableObject, NarrativeSceneConfig serializable class, SceneType enum, ConditionType enum, GoalUrgency enum"
      contains: "CreateAssetMenu"
    - path: "Assets/AyaLiveStream/Data/Beat_WarmUp.asset"
      provides: "Beat 1: warm-up (getting to know Aya), urgency: Low"
    - path: "Assets/AyaLiveStream/Data/Beat_ArtProcess.asset"
      provides: "Beat 2: art process discussion, urgency: Medium"
    - path: "Assets/AyaLiveStream/Data/Beat_Characters.asset"
      provides: "Beat 3: character discussion leading to movie reveal, urgency: High"
  key_links:
    ...
    - from: "NarrativeBeatConfig"
      to: "GoalUrgency"
      via: "urgency field"
      pattern: "public GoalUrgency urgency"
---

<objective>
Create the NarrativeBeat/Scene data model as ScriptableObjects and author 3 beat assets (warm-up, art process, characters) inspired by the nevatars beat system but simplified for the demo. Each beat carries a GoalUrgency level (Low, Medium, High) representing escalating narrative intensity across the arc.

Purpose: All other plans in this phase depend on the NarrativeBeatConfig type. The beat assets define the narrative arc content that NarrativeDirector will execute. The urgency field drives escalating pacing and director note intensity across beats.

Output: NarrativeBeatConfig.cs with embedded NarrativeSceneConfig, SceneType enum, ConditionType enum, GoalUrgency enum, plus 3 authored .asset files in Assets/AyaLiveStream/Data/.
</objective>
...
<task type="auto">
  ...
  <action>
Create NarrativeBeatConfig.cs in the AIEmbodiment.Samples namespace with these public types:

**GoalUrgency enum**: Low, Medium, High (3 values)
- Represents escalating narrative intensity across the beat arc
- Low = casual/warm-up beats, Medium = engaged/building beats, High = climactic/reveal beats
- Used by NarrativeDirector to influence director note phrasing and beat pacing
...
- `[Tooltip] GoalUrgency urgency` (default GoalUrgency.Low) -- "Urgency level for this beat. Escalates across the narrative arc (Low -> Medium -> High). Influences director note phrasing and beat pacing."
...
Do NOT include: exit patterns, alternate consequences, ConsequenceType enum, repeat limits, pre-recorded audio, option pairs, poll options, animation triggers, mood states. These are nevatars over-engineering that the 3-beat demo does not need. Beats are linear (1->2->3) so no jump targets.
  ...
  <verify>
File compiles: open Assets/AyaLiveStream/NarrativeBeatConfig.cs and verify it has no syntax errors, uses correct namespace (AIEmbodiment.Samples), and has the CreateAssetMenu attribute. Count: 1 SO class, 1 Serializable class, 3 enums (SceneType, ConditionType, GoalUrgency).
  </verify>
  <done>NarrativeBeatConfig.cs exists with NarrativeBeatConfig (SO), NarrativeSceneConfig (Serializable), SceneType (5 values), ConditionType (3 values), GoalUrgency (3 values: Low, Medium, High). All fields have Inspector-friendly attributes. GoalUrgency field is present on NarrativeBeatConfig with tooltip explaining its role in escalation.</done>
</task>
...
**Beat 1: Warm-Up** (beatId: "BEAT_WARMUP", timeBudget: 180s = 3 min, urgency: GoalUrgency.Low)
...
**Beat 2: Art Process** (beatId: "BEAT_ART", timeBudget: 240s = 4 min, urgency: GoalUrgency.Medium)
...
**Beat 3: Characters** (beatId: "BEAT_CHARACTERS", timeBudget: 180s = 3 min, urgency: GoalUrgency.High)
...
Editor script compiles and menu item "AI Embodiment/Create Demo Beat Assets" appears. Alternatively, if writing raw .asset files, verify YAML structure is valid.
  ...
  <done>3 beat assets are creatable via editor menu (or exist as .asset files). Each has distinct timeBudget, goalDescription, directorNote, urgency level (Low, Medium, High), and 3 ordered scenes with appropriate scene types and condition types. The urgency escalates across the 3 beats: warm-up=Low, art=Medium, characters=High.</done>
</task>
...
4. GoalUrgency enum has exactly 3 values: Low, Medium, High
5. NarrativeBeatConfig has CreateAssetMenu attribute, urgency field, and all required fields
6. Editor script (or .asset files) produces 3 beat assets with correct content and escalating urgency
7. No nevatars over-engineering: no exit patterns, no ConsequenceType, no repeat limits, no pre-recorded audio
8. MIG-02 coverage: Nevatars beat/scene data is used as inspiration, not migrated as-is (per CONTEXT.md decision). The mapping is: beat structure preserved (beatId, title, scenes array), scene ordering preserved (sequential scene list), cue timing preserved via timeBudgetSeconds + conditional maxDuration, scripted messages preserved via dialogueAlternatives. Dropped: exit patterns, alternate consequences, repeat limits, pre-recorded audio, Login scene type, 5 of 8 condition types. This is a deliberate redesign using nevatars as a reference, not a migration.
</verification>

<success_criteria>
- NarrativeBeatConfig type is available for NarrativeDirector (14-02) to reference
- GoalUrgency enum exists with Low, Medium, High values for escalating narrative intensity
- 3 beat assets define the complete narrative arc content for the 10-minute demo with escalating urgency
- Data model is Inspector-friendly with Header/Tooltip/TextArea attributes
- Simplified from nevatars (5 scene types, 3 condition types, 3 urgency levels vs 6/8/0 in nevatars)
</success_criteria>
