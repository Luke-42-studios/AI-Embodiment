...
must_haves:
  ...
  artifacts:
    - path: "Assets/AyaLiveStream/NarrativeDirector.cs"
      provides: "Beat lifecycle loop, SendText steering, dual-queue coordination events, IsAyaSpeaking flag"
...
<objective>
Create NarrativeDirector MonoBehaviour with time-based beat lifecycle, SendText director note steering, and dual-queue coordination events. Modify ChatBotManager to integrate pacing control (slower bursts when Aya speaks, pause at beat transitions).

Purpose: This is the core orchestrator for the narrative arc. It drives beat progression, steers Aya via SendText, and provides coordination signals for all other systems. The PTT controller (14-04) and scene execution (14-03) both depend on this foundation.

Output: NarrativeDirector.cs with beat loop + events, modified ChatBotManager.cs with pacing integration.
</objective>
...
Follow the async Awaitable pattern established by ChatBotManager.ScriptedBurstLoop: try/catch OperationCanceledException, use destroyCancellationToken.
  ...
  <verify>
NarrativeDirector.cs compiles. Verify: has StartNarrative(), RunBeatLoop(), IsAyaSpeaking property, OnBeatTransition event, SendText call guarded by _isAyaSpeaking check, CheckSkipKeywords method, and OnDestroy cleanup.
  </verify>
  <done>NarrativeDirector drives beat progression with time + goal, sends director notes safely (never while Aya speaks), exposes IsAyaSpeaking and beat events for downstream systems.</done>
</task>
...
2. Beat loop progresses through _beats array with time-based advancement
3. SendText fires ONLY after checking !_isAyaSpeaking (pending queue if Aya speaking)
4. OnBeatTransition event fires between beats for sync
5. ChatBotManager integrates NarrativeDirector pacing without breaking existing burst/dynamic response APIs
6. Skip keyword detection works via OnInputTranscription subscription
7. All async methods use destroyCancellationToken and try/catch OperationCanceledException
8. OnDestroy properly unsubscribes all events
</verification>
