...
must_haves:
  truths:
    ...
    - "If Aya is speaking when SPACE is pressed, microphone recording is deferred until Aya's turn completes (finish-first pattern)"
  artifacts:
    - path: "Assets/AyaLiveStream/PushToTalkController.cs"
      provides: "4-state PTT machine (Idle, Recording, Reviewing, Submitted), finish-first logic with deferred StartListening, transcript approval"
  ...
  key_links:
    ...
    - from: "PushToTalkController.cs"
      ...
      via: "microphone control during recording state -- deferred if Aya speaking"
      pattern: "StartListening|StopListening"
    - from: "PushToTalkController.cs"
      to: "NarrativeDirector.OnTurnComplete (via PersonaSession)"
      via: "deferred recording trigger after Aya finishes"
      pattern: "OnTurnComplete.*StartListening|_deferredRecording"
---
...
NOTE: WaitingForAya is a sub-state of the Recording concept. The user has pressed SPACE and intends to record, but since Aya is still speaking, we defer StartListening until OnTurnComplete fires. This implements the "finish-first" pattern: Aya finishes her current thought, THEN the microphone opens. This is CRITICAL because sending audio to Gemini while Aya is speaking may trigger an interruption and clear Aya's audio buffer.
...
**Private state:**
- `PTTState _state = PTTState.Idle` (private enum: Idle, WaitingForAya, Recording, Reviewing, Submitted)
- `string _transcript = ""`
...
1. If `_state == WaitingForAya`: Aya has finished speaking, NOW start recording.
   - Call `_session.StartListening()`
   - Set `_state = PTTState.Recording`
   - `_livestreamUI.SetPTTStatus("Recording...", active: true)`
2. If `_state == Submitted`: Aya has finished responding to the user's input.
   - Reset to Idle state
...
- FINISH-FIRST PATTERN: When Aya is speaking and user presses SPACE, do NOT call StartListening. Instead enter WaitingForAya state and show acknowledgment. When OnTurnComplete fires, THEN call StartListening. This prevents audio from being sent to Gemini while Aya is speaking, which would trigger an interruption and clear her audio buffer. If the user releases SPACE before Aya finishes, cancel the deferred recording and return to Idle.
...
- Chat keeps flowing during PTT -- PushToTalkController does NOT pause ChatBotManager (per CONTEXT.md: "Only Aya pauses during PTT").
  ...
  <verify>
PushToTalkController.cs compiles. Verify: 5-state machine (Idle, WaitingForAya, Recording, Reviewing, Submitted), EnterRecording checks IsAyaSpeaking and defers StartListening if Aya is speaking, HandleTurnComplete transitions WaitingForAya to Recording with StartListening call, SubmitTranscript has idempotent guard, OnDestroy unsubscribes events, uses Keyboard.current for input.
  </verify>
  <done>PushToTalkController implements finish-first PTT with: WaitingForAya state that defers StartListening until Aya's turn completes (preventing Gemini audio interruption), immediate acknowledgment when Aya is speaking (<500ms), transcript review overlay with 3-second auto-submit countdown, Enter to send immediately, Escape to cancel silently, idempotent submission guard, and proper event lifecycle.</done>
</task>
...
2. StartListening is NOT called while Aya is speaking -- deferred to OnTurnComplete via WaitingForAya state
3. If user releases SPACE before Aya finishes, deferred recording is cancelled and state returns to Idle
4. SPACE release (after recording) shows transcript overlay with countdown bar
5. Auto-submit fires after 3 seconds with no user action
6. Enter submits immediately (cancels timer implicitly via state transition)
7. Escape cancels silently (no animation, no feedback)
8. User message appears in chat feed on submit
9. No double-submission from Enter + timer race condition
10. PTT does NOT pause ChatBotManager (chat keeps flowing)
11. All events properly subscribed in Start and unsubscribed in OnDestroy
12. Transcript overlay styles match dark theme (rgba(24,24,40,0.95) background)
</verification>

<success_criteria>
- Visual acknowledgment appears within 500ms (same-frame CSS toggle)
- Aya finishes her current response before microphone opens (finish-first via deferred StartListening)
